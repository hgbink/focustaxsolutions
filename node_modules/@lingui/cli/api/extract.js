...
function extract(srcPaths, targetPath) {
  ...
  srcPaths.forEach(function (srcFilename) {
    ...
    if (_fs2.default.lstatSync(srcFilename).isDirectory()) {
    ...
    if (extracted && verbose) console.log(_chalk2.default.green(srcFilename));
  });
}
function collect(buildDir) {
  return _fs2.default.readdirSync(buildDir).map(function (filename) {
    ...
    var mergeMessage = function mergeMessage(msgId, prev, next) {
